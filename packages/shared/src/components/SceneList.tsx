...
import type { Chunk, AISettings } from '../types';
import { useNavigation, useStorage } from '../contexts/PlatformContext';
...
const SceneList = () => {
    ...
    const checkAndRedirectAPISettings = async (aiSettings: AISettings | undefined): Promise<boolean> => {
        if (!aiSettings ||
            !aiSettings.apiKey ||
            !aiSettings.apiUrl ||
    ...
    const handleSceneClick = async (sceneId: string) => {
        ...
        await checkAndRedirectAPISettings(settings.ai);
    };
    ...
    const handleSceneSubmit = async () => {
        ...
        const settings = JSON.parse(savedSettings);
        if (!await checkAndRedirectAPISettings(settings.ai)) {
            return;
        }
